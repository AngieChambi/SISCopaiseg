...
from apps.sad.menus import Menus, MenusClivet
...
@register.simple_tag
def load_menu_clivet(request, module):
    """
    Interfáz del Método para cargar en variables los menús que se mostrará al usuario

    Usage::

            {% load_menu_clivet request 'MODULE_KEY' %}
    ...
    Examples::
            {% load_menu_clivet request 'CLINICA' %}

    """

    return MenusClivet.load(request, module)


@register.simple_tag
def view_menu_clivet(request):
    """
    Interfáz del Método para renderizar el menú
    Usage::

            {% view_menu_clivet request %}
    ...
    return mark_safe(MenusClivet.view(request))
