...
class UserListView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['opts'] = self.model._meta
        if UserToken.get_headquar_id(self.request.session):
            empresa = Headquar.objects.get(
                id=UserToken.get_headquar_id(self.request.session))

            hola =   User.objects.filter(userheadquar__headquar__id = empresa.id)
            context['lista_user'] = Counter(hola).keys()
            print("empresa", User.objects.filter(userheadquar__headquar__id = empresa.id))
        else:
            context['lista_user'] = User.objects.all()
        context['cmi'] = 'user'
