...
class MenusClivet:
    ...
    """
    menu_module = ''  # Variable para indicar el entorno
    menu_list = []  # Variable que contiene los menusclivet
    menu_item_list = {}  # Variable que contien los items del menusclivet
    ...
    def load(request, menu_module):
        ...
        Salida::
                menu_item_list[menu]
        """
        MenusClivet.menu_list = []
        MenusClivet.menu_item_list = {}
        MenusClivet.menu_module = menu_module
        user = request.user
        ...
        MenusClivet.menu_list = Menu.objects.filter(
            menu__in=menu_item_list_t, module=menu_module, is_active=True).order_by("pos").distinct()
        #print MenusClivet.menu_list
        if MenusClivet.menu_list:
            for menu in MenusClivet.menu_list:
                MenusClivet.menu_item_list[menu.title] = Menu.objects.filter(Q(permission__in=permission_list) | Q(
                    id__isnull=True if permission_list else False), parent_id=menu.id, module=menu_module, is_active=True).order_by("pos")  # .lower().replace(" ","_")
        #print MenusClivet.menu_item_list
        return ""
    ...
    def desktop(request):
        ...
        if MenusClivet.menu_list:
            ...
            for main in MenusClivet.menu_list:
                active = ('active' if main.url == route else '')
                html = html + '<li class="%s">%s</li>\n' % (
                    active, MenusClivet.linkdf(main.url, main.title, main.icon))
            html = html + '</ul>\n'
        return html
    ...
    def linkphone(action, text, icon):  # , attrs = None, icon='', loadAjax=True
        ...
        texti = ""
        if icon:
            texti = '<i class="menu-icon %s fa-lg"></i>' % icon
        html = '<a href="%s" class="dropdown-toggle" data-toggle="dropdown">%s %s</a>\n' % (
            action, texti, text)
        return html
    ...
    def desktop_items(request):
        ...
        html = ''
        route = request.path
        for menu, items in MenusClivet.menu_item_list.iteritems():
            html = html + \
                '<div id="sub-menu-%s" class="subnav hidden">\n' % menu.lower().replace(
                    " ", "_")
            html = html + '<ul class="nav nav-pills">\n'

            if menu in MenusClivet.menu_item_list:
                for item in MenusClivet.menu_item_list[menu]:
                    active = ('active' if item.url.strip(
                        "/") == route.strip("/") else '')
                    html = html + '<li class="%s">%s</li>\n' % (
                        active, MenusClivet.link(item.url, item.title, item.icon))
            html = html + '</ul>\n'
            html = html + '</div>\n'
        return html
    ...
    def view(request):
        ...
        route = request.path
        if MenusClivet.menu_list:
            html = html + '<ul class="nav nav-list">\n'
            for main in MenusClivet.menu_list:
                text = '<span class="menu-text">%s</span><b class="arrow fa fa-angle-down"></b>' % capfirst(_("%s" % main.title))
                html = html + '<li class="%s">\n'
                html = html + MenusClivet.linkphone('#', text, main.icon)
                if main.title in MenusClivet.menu_item_list:
                    html = html +'<b class="arrow"></b>'
                    html = html + '<ul class="submenu">\n'
                    for item in MenusClivet.menu_item_list[main.title]:
                        active = ('active' if item.url.strip("/") == route.strip("/") else '')
                        html = html + '<li class="%s">%s<b class="arrow"></b></li>\n' % (
                            active, MenusClivet.link(item.url, item.title, item.icon))
                    html = html + '</ul>\n'
                html = html + '</li>\n'
            html = html + '</ul>\n'
            ...
            if menu in MenusClivet.menu_item_list:
                for item in MenusClivet.menu_item_list[menu]:
                    active = ('active' if item.url.strip(
                        "/") == route.strip("/") else '')
                    html = html + '<li class="%s">%s</li>\n' % (
                        active, MenusClivet.link_side(item.url, item.title, item.icon))
